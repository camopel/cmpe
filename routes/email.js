				...
				if(err!=null) res.send({"success":"false","msg":err});
					...
					db.collection('projects').find({"sjsuid": {$in:recv}}).toArray(function(err,data){
						if(err!=null) res.send({"success":"false","msg":err});
							...
							db.collection('conversions').find({"sjsuid": {$in:recv}}).toArray(function(err,convs){
								if(err!=null) res.send({"success":"false","msg":err});
								else{									
									for(var i=0;i<convs.length;i++){
										us[convs[i].sjsuid][3]+=parseFloat(convs[i].conversion);										
									}
									for(var i=0;i<recv.length;i++){
										var id = recv[i];
										var to = us[id][2];
										var sub = req.body.sub;
										var body = req.body.html.replace("#fullname#",us[id][0]).replace("#lastname#",us[id][1]).replace("#totalpoints#",us[id][3].toFixed(2));								
